...
bool TestManager::parseCommandLine(const int argc, const char* argv[]) {
  bool result = false;
...
void TestManager::add(TestAbleClass* testClass) {
  testClasses_.push_back(testClass);
}


void TestManager::sortTests() {
...
void TestManager::countTestsAndBatches() {
  numberOfBatches_ = 0;
  numberOfTests_ = 0;
  for(auto& test : tests_) {
    if( typeid(*test) == typeid(Batch) ) {
...
void TestManager::listAllBatches() {
  registerTests();
  sortTests();
  countTestsAndBatches();
...
bool TestManager::runTests(const std::vector<std::string>& testNames) {
  Batch testBatch{"TestBatch", false};
...
void TestManager::registerTests() const {
