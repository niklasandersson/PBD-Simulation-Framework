...
__kernel void main(
  ...
  const unsigned int read_right = ((group_id/step)+1)*2*step - group_id%step -1; 

  const unsigned int write_left = read_left;  
  const unsigned int write_right = read_left + step; 
  
  if( (read_left*(BLOCK_SIZE/2) + local_id < numberOfElements) &&
      (read_right*(BLOCK_SIZE/2) + (BLOCK_SIZE/2) -1 - local_id < numberOfElements) ) {

    const unsigned int sml = input[read_left*(BLOCK_SIZE/2) + local_id];
    const unsigned int smb = input[read_right*(BLOCK_SIZE/2) + (BLOCK_SIZE/2) -1 - local_id];
